...
#define REPORT_DEAD 				"死亡"
#define RELATIONSHIP_NONE 			"無"
#define RELATIONSHIP_ENEMY 			"联盟敵人!"
#define RELATIONSHIP_FRIEND 		"联盟队友"
#define RELATIONSHIP_NEUTRAL 		"路人"
#define RELATIONSHIP_UNSURE			"未知"
...
public OnPluginStart(){
	...
	g_cvar_enable = CreateConVar("sm_shownpchp_enable", "1", "是否顯示血液 (0=off/1=on)", FCVAR_PLUGIN);
	g_cvar_hud = CreateConVar("sm_shownpchp_hud", "1", "顯示位置 0=HudText, 1=HintText", FCVAR_PLUGIN);
...
public ShowHPInfo(client, target) {
	...
	switch(GetConVarInt(g_cvar_hud)){
		case 1:{
			...
			PrintHintText(client,"%s (%s)\n你們的關係: %s",targetname,strHealth,relationship);
		}
		case 0:{
