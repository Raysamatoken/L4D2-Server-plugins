...
public OnPluginStart()
{
	...
	g_hDT_rate = CreateConVar(
		...
		"0.8" ,
		"原本射速×设定值. 警告: 太快会导致射击精度降低 (设定值 0.2 < 2.0)" ,
		FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.2, true, 2.0);
	...
	if (GetConVarBool(powerups_plugin_on))
	{
		if (GetConVarBool(powerups_broadcast_on))
		{
			if (GetConVarBool(powerups_use_on))
			{
				...
				{
					PrintToChat(client, "\x03[\x04提示\x03] \x05本服务器,在使用药丸后可能(针管100％)触发装弹+射速+近战挥舞速度加快");
				}
				...
				{
					PrintToChat(client, "\x03[\x04提示\x03] \x05本服务器在触发药丸和针管的特殊能力后会有时间限制(默认20秒)");
			...
			else if (!GetConVarBool(powerups_use_on))
			{
				if (GetConVarInt(powerups_broadcast_type) == 0)
				{
					PrintToChat(client, "\x03[\x04提示\x03] \x05本服务器,使用药丸后可能(针管100％)触发 装弹+射速+近战挥舞速度加快!");
				}
				else if (GetConVarInt(powerups_broadcast_type) == 1)
				{
...
public GiveAdrenalineToAll()
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2)
		{
			...
			PrintToChat(i, "\x03[\x04提示\x03] \x05获得 \x04针管");
		}
	}
	SetCommandFlags("give", flags|FCVAR_CHEAT);
}
...
public GivePillsToAll()
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2)
		{
			...
			PrintToChat(i, "\x03[\x04提示\x03] \x05获得 \x04药丸");
		}
	}
	SetCommandFlags("give", flags|FCVAR_CHEAT);
}
...
public Action:Timer_UsedHealth(Handle:Timer, any:client)
{
	if (GetConVarBool(powerups_use_on))
	{
		PrintToChat(client, "\x03[\x04提示\x03] \x05装弹+射速+近战挥舞速度加快!");
...
public Action:Timer_EndPower(Handle:Timer, any:client)
{
	if (GetConVarBool(powerups_use_on))
	{
		PrintToChat(client, "\x03[\x04提示\x03] \x05装弹+射速+近战挥舞速度恢复正常");
		g_usedhealth[client] = 0
		RebuildAll();
	}
}
