...
public OnPluginStart()
{
	...
	cvarInvulnerable = CreateConVar("l4d2_dlock_godmode", "0", "幸存者冻结/锁定无伤");
	cvarFreezeNodoor = CreateConVar("l4d2_dlock_freezenodoor", "0", "无安全门冻结幸存者");
	cvarPrepareTime1r = CreateConVar("l4d2_dlock_prepare1st", "10", "等待游戏时间");
	cvarPrepareTime2r = CreateConVar("l4d2_dlock_prepare2nd", "5", "重启等待游戏时间");
	cvarWaitForInfected = CreateConVar("l4d2_dlock_infectedspawn", "0", "等待被感染者");
	cvarClientTimeOut = CreateConVar("l4d2_dlock_timeout", "180", "网络超时客户等待最大限度时长");
	cvarBreakTheDoor = CreateConVar("l4d2_dlock_weakdoor", "0", "安全门被打开后自行坏掉");
	cvarStartPrevent = CreateConVar("l4d2_dlock_startprevent", "1", "安全门打开后回合开始");
	cvarDisplayPanel = CreateConVar("l4d2_dlock_displaypanel", "2", "玩家加载状态面板（0:关闭 1:显示加载成功的 2:显示全部）");
	cvarDisplayMode = CreateConVar("l4d2_dlock_displaymode", "hint", "提示框显示位置(hint:中下方 center:中间 chat:聊天框)");
	cvarReadyUpMode = CreateConVar("l4d2_dlock_readyupmode", "0", "安全门解锁方式(0:定时解锁 1:手动解锁");
	cvarGameModeEnabled = CreateConVar("l4d2_dlock_gamemodeactive", "coop", "支持的游戏模式");
