...
public OnPluginStart()
{
	...
	CreateConVar("countrynick_version", VERSION, "本插件版本", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
}
...
public OnClientPutInServer(client)
{
	...
	if(!IsFakeClient(client) && client != 0)
	{
		...
		else
		{
			...
			LogError("[Country Nick] Warning : %s使用%s 那不是列在GEOIP资料库内", name, ip);
		}
	}
}
