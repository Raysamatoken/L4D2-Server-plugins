...
{	
	...
	// Enable Cvar
	new Handle:Enabled = CreateConVar("l4d_csm_enable", "1", "开启插件", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);
	g_bEnabled = GetConVarBool(Enabled);
	HookConVarChange(Enabled, _ConVarChange__Enable);
	
	// Admin cvar
	new Handle:AdminsOnly = CreateConVar("l4d_csm_admins_only", "0","所有人可用. 1 = 只有管理员可用.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 1.0);
	g_bAdminsOnly = GetConVarBool(AdminsOnly);
	HookConVarChange(AdminsOnly, _ConVarChange__AdminsOnly);
	
	// Announce cvar
	new Handle:Announce = CreateConVar("l4d_csm_announce", "1","显示消息.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 1.0);
	g_bAnnounce = GetConVarBool(Announce);
	HookConVarChange(Announce, _ConVarChange__Announce);
	
	// Survivor cvar
	new Handle:SurvivorModels = CreateConVar("l4d_csm_survivor_access", "1","1 = 改变属性. 2 = 改变模型. 3 = 两者都改变.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 3.0 );
	g_iSurvivorModels = GetConVarInt(SurvivorModels);
	HookConVarChange(SurvivorModels, _ConVarChange__SurvivorModels);
	
	// Infected cvar
	new Handle:InfectedModels = CreateConVar("l4d_csm_infected_access", "0", "1 = 只改变属性. 2 = 只改变模型. 3 = 改变属性和模型.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 3.0);
	...
	{
		// L4D1 Survivors cvar
		new Handle:L4D1Survivors = CreateConVar("l4d_csm_l4d1_survivors", "1","允许使用L4D1的人物. L4D2请设置为1, 如果是L4D1设置为2避免错误",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 2.0);
		...
		new Handle:CommonModels = CreateConVar("l4d_csm_common_infected_models_access", "0", "可用使用感染者模型", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 3.0);
		g_bCommonModels = GetConVarBool(InfectedModels);
		HookConVarChange(CommonModels, _ConVarChange__CommonModels);
	}
	
	// Change Limit Cvar
	new Handle:ChangeLimit = CreateConVar("l4d_csm_change_limit", "9999","改变模型时间/秒.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0);
	...
	if (IsClientInGame(client)) 
	{
		if (GetClientTeam(client) == TEAM_SURVIVORS && g_bAnnounce)
		{
			switch (g_iSurvivorModels) 
			{
				...
				PrintToChat(client, "\x03[CSM]\x01 聊天栏输入 \x05!csm\x01 改变你的外形 .");
...
public Action:ShowMenu(client, args) 
{
	...
	{
		ReplyToCommand(client, "只有管理员可用模型选单.");
		return;
	}
	if (GetClientTeam(client) != TEAM_SURVIVORS)
	{
		ReplyToCommand(client, "只能使用幸存者模型.");
		return;
	}
	if (g_iSurvivorModels == 0 && g_iInfectedModels == 0 && !g_bCommonModels)
	{
		ReplyToCommand(client, "无可用选项.");
		return;
	}
	if (!PlayerIsAlive(client)) 
	{
		ReplyToCommand(client, "必须活着时使用模型选单!");
		return;
	}
	if (g_iClientChangeLimit[client] < 1) 
	{
		ReplyToCommand(client, "你现在不能使用模型选单.");
		return;
	}
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE) 
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 必须活着时使用模型选单.", g_sCharacters[Survivor]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x051\x01 选单.", g_sCharacters[Survivor]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x05%i\x01 选单.", g_sCharacters[Survivor], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01!", g_sCharacters[Survivor]);
		}
	}
}
...
stock SetUpSurvivorModel(param1, Survivor) 
{
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE)
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 必须活着时使用模型选单.", g_sCharacters[Survivor+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x051\x01 选单.", g_sCharacters[Survivor+8]);
			else
		PrintToChat(param1, "\x03[CSM]\x01 Y你现在的人物是 \x04%s\x01! 你可以使用 \x05%i\x01 选单.", g_sCharacters[Survivor+8], g_iClientChangeLimit[param1]);
		else
		PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01!", g_sCharacters[Survivor+8]);
	}
}
...
stock SetUpInfected(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 必须活着时使用模型选单.", g_sCharacters[Infected]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x051\x01 选单.", g_sCharacters[Infected]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x05%i\x01 选单.", g_sCharacters[Infected], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01!", g_sCharacters[Infected]);
		}
	}
...
stock SetUpInfectedModel(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 必须活着时使用模型选单.", g_sCharacters[Infected+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x051\x01 选单.", g_sCharacters[Infected+8]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01! 你可以使用 \x05%i\x01 选单.", g_sCharacters[Infected+8], g_iClientChangeLimit[param1]);
		}
		else
		{
			PrintToChat(param1, "\x03[CSM]\x01 你现在的人物是 \x04%s\x01!", g_sCharacters[Infected+8]);
		}
	}
}
