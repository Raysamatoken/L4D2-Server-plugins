...
{ 	
			 ...
			 { 	
			 decl String:GameName[16]; 	GetConVarString(FindConVar("mp_gamemode"), GameName, sizeof(GameName)); 	 	 	
			 if (StrEqual(GameName, "survival", false)) 		
			 GameMode = 3; 	else if (StrEqual(GameName, "versus", false) || StrEqual(GameName, "teamversus", false) || StrEqual(GameName, "scavenge", false) || StrEqual(GameName, "teamscavenge", false)) 		
			 GameMode = 2; 	else if (StrEqual(GameName, "coop", false) || StrEqual(GameName, "realism", false)) 		
			 GameMode = 1; 	else 	{ 		GameMode = 0;  	} 	GameMode=GameMode+0; 	
...
{	  	new Handle:menu = CreateMenu(MenuSelector2); 	 	
         ...
         if(give)AddMenuItem(menu, "Give", "Give me a shield on back"); 	SetMenuExitButton(menu, true);  	
...
public MenuSelector2(Handle:menu, MenuAction:action, client, param2) { 	if(client>0 && IsClientInGame(client) && IsPlayerAlive(client) && GetClientTeam(client)==2) 	
 { 		
	   ...
	   DispatchSpawn(ent); 				TeleportEntity(ent, pos, angle, NULL_VECTOR);		 				
...
public Action:BuildShieldTimer(Handle:timer, any:client) 
    { 	
	 ...
	 {  			CreateShield(client);  			
	 RemoveShieldWeapon(client); 			return Plugin_Stop; 		
	 } 	
	 } 	
	 else 	
...
{  	new client = GetClientOfUserId(GetEventInt(hEvent, "userid"));    	if(client>0) 	{		 		ResetClientState(client); 		if(GetClientTeam(client)==3)SpawnShieldTank(client);	 	} }    public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast) {  	new client = GetClientOfUserId(GetEventInt(hEvent, "userid"));  	if(client>0 && client<=MaxClients) 	{ 		if(IsMelee(MeleeEnt[client]) && GetRandomFloat(0.0, 100.0)<GetConVarFloat(l4d_shield_chance_drop)) 		{ 			new Float:pos[3]; 			new Float:angle[3]; 			GetClientEyePosition(client, pos); 			GetClientAbsAngles(client, angle); 			  			new ent=CreateEntityByName("weapon_melee");  			DispatchKeyValue( ent, "melee_script_name", "riotshield");  			DispatchSpawn(ent); 			TeleportEntity(ent, pos, angle, NULL_VECTOR);		 			SetEntPropFloat(ent , Prop_Send,"m_flModelScale", 1.5);   		} 		DeleteBackShield(client); 		ResetClientState(client); 	} 	  } public player_bot_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast) {  	new client = GetClientOfUserId(GetEventInt(Spawn_Event, "player")); 	new bot = GetClientOfUserId(GetEventInt(Spawn_Event, "bot"));    	  	if(client>0) 	{ 		DeleteBackShield(client);	 		ResetClientState(client); 	} 	if(bot>0) 	{  		ResetClientState(bot); 		SpawnShieldTank(bot);  	} } public bot_player_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast) {  	new client = GetClientOfUserId(GetEventInt(Spawn_Event, "player")); 	new bot = GetClientOfUserId(GetEventInt(Spawn_Event, "bot"));    	  	if(bot>0) 	{  		ResetClientState(bot); 	} 	if(client>0) 	{ 		 DeleteBackShield(client); 		 ResetClientState(client); 		 SpawnShieldTank(client); 	}  }   DeleteBackShield(client) { 	if(client<=0)return; 	if(IsMelee(MeleeEnt[client])) 	{ 		AcceptEntityInput(MeleeEnt[client], "ClearParent"); 		AcceptEntityInput(MeleeEnt[client], "kill"); 		
...
{     
		...
		LastButton[client]=b; 	}  } public Action:PlayerOnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype) { 	if(victim<=0)return Plugin_Continue; 	
		new mode=ShieldMode[victim]; 	if(mode==ShieldMode_None) 	{ 		SDKUnhook(victim, SDKHook_OnTakeDamage,  PlayerOnTakeDamage); 		
		return Plugin_Continue; 	}	 	if(mode & ShieldMode_Front) 	{   		new ent=HoldShieldWeapon(victim); 		if(ent<=0) 		
		{ 			mode =mode & ~ShieldMode_Front; 		}  		else ShieldWeaopn[victim]=ent; 	}    	if(mode==ShieldMode_None)return Plugin_Continue; 	
		decl Float:attackerPos[3]; 	new Float:damageFactor=100.0; 	if(attacker>0 && attacker<=MaxClients) 	{ 		GetClientAbsOrigin(attacker, attackerPos);  		
		if(GetEntProp(attacker, Prop_Send, "m_zombieClass")==ZOMBIECLASS_TANK)damageFactor=GetConVarFloat(l4d_shield_damage_from_tankwitch); 		
			 ...
			 return Plugin_Continue; } Float:GetAngle(Float:x1[3], Float:x2[3]) 
			 { 	
			 ...
			 SetEntityModel(melee, "models/weapons/melee/w_riotshield.mdl"); 	
			 DispatchSpawn(melee);  	
			 SetEntPropFloat(melee , Prop_Send,"m_flModelScale",scale);   	 	new Float:ang[3];	 	
			 new Float:pos[3]; 	SetVector(pos, 0.0, 14.0,  7.0);    	
			 SetVector(ang, 180.0, 0.0,  90.0);    	 	SetEntityMoveType(melee, MOVETYPE_NONE);		  	
			 SetEntProp(melee, Prop_Data, "m_CollisionGroup", 2);    	
			 AttachEnt(client, melee, "medkit", pos, ang);   	 	
			 MeleeEnt[client]=melee;	 	
			 MeleeEnt2[client]=0;  	ShieldMode[client]|=ShieldMode_Back; 	 	
			 SDKUnhook(client, SDKHook_OnTakeDamage,  PlayerOnTakeDamage); 	
			 SDKHook(client, SDKHook_OnTakeDamage,  PlayerOnTakeDamage); 	
...
public OnMapStart() 
		...
		ShowLaser(colortype,Float:pos1[3], Float:pos2[3], Float:life=10.0,  Float:width1=1.0, Float:width2=11.0) { 	decl color[4]; 	
		if(colortype==1) 	
		{ 		
		color[0] = 200;  		
		color[1] = 0; 		
		...
		//draw line start from pos, the line's drection is dir. ShowDir(color,Float:pos[3], Float:dir[3],Float:life=10.0, Float:length=200.0, Float:width1=1.0, Float:width2=11.0) 
		{ 	decl Float:pos2[3]; 	
		CopyVector(dir, pos2); 
		...
		//draw line start from pos, the line's angle is angle. ShowAngle(color,Float:pos[3], Float:angle[3],Float:life=10.0, Float:length=200.0, Float:width1=1.0, Float:width2=11.0) 
		{ 	
		decl Float:pos2[3]; 	
