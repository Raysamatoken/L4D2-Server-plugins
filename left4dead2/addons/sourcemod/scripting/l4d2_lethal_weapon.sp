...
{
	// ConVars
	l4d2_lw_lethalweapon	= CreateConVar("l4d2_lw_lethalweapon","1", "", CVAR_FLAGS);
	l4d2_lw_lethaldamage	= CreateConVar("l4d2_lw_lethaldamage","3000.0", "", CVAR_FLAGS);
	l4d2_lw_lethalforce		= CreateConVar("l4d2_lw_lethalforce","500.0", "", CVAR_FLAGS);
	l4d2_lw_chargetime		= CreateConVar("l4d2_lw_chargetime","10", "", CVAR_FLAGS);
	l4d2_lw_shootonce		= CreateConVar("l4d2_lw_shootonce","0", "", CVAR_FLAGS);
	l4d2_lw_ff				= CreateConVar("l4d2_lw_ff","0", "", CVAR_FLAGS);
	l4d2_lw_scout			= CreateConVar("l4d2_lw_scout","1", "", CVAR_FLAGS);
	l4d2_lw_awp				= CreateConVar("l4d2_lw_awp","1", "", CVAR_FLAGS);
	l4d2_lw_huntingrifle	= CreateConVar("l4d2_lw_huntingrifle","1", "", CVAR_FLAGS);
	l4d2_lw_g3sg1			= CreateConVar("l4d2_lw_g3sg1","1", "", CVAR_FLAGS);
	l4d2_lw_laseroffset		= CreateConVar("l4d2_lw_laseroffset", "36", "", FCVAR_NOTIFY);
	
	// Additional ConVars
	l4d2_lw_flash				= CreateConVar("l4d2_lw_flash", "1", "");
	l4d2_lw_chargingsound		= CreateConVar("l4d2_lw_chargingsound", "1", "");
	l4d2_lw_chargedsound		= CreateConVar("l4d2_lw_chargedsound", "1", "");
	l4d2_lw_moveandcharge		= CreateConVar("l4d2_lw_moveandcharge", "1", "");
	l4d2_lw_chargeparticle		= CreateConVar("l4d2_lw_chargeparticle", "1", "");
	l4d2_lw_useammo				= CreateConVar("l4d2_lw_useammo", "0", "");
	l4d2_lw_shake				= CreateConVar("l4d2_lw_shake", "1", "");
	l4d2_lw_shake_intensity		= CreateConVar("l4d2_lw_shake_intensity", "50.0", "");
	l4d2_lw_shake_shooteronly	= CreateConVar("l4d2_lw_shake_shooteronly", "0", "");
...
public Action:Event_Weapon_Fire(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (ReleaseLock[client])
	{
		...
		{
			ChargeLock[client] = 99999;
			PrintHintText(client, "等离子武器每关只可使用一次");
		}
		else
		{
...
public Action:ChargeTimer(Handle:timer, any:client)
{
	...
	if (inCharge && GetEntData(WeaponClass, ClipSize))
	{
		if (ChargeEndTime[client] < gt)
		{
			...
			PrintCenterText(client, "*****************[黑客提示]:充电完毕*****************");
		...
		else
		{
			...
			/* Display gauge */
			PrintCenterText(client, "           <<[黑客提示]:充电进行中>>\n0%% %s %3.0f%%", ChargeBar, GaugeNum*2);
		}
	}
	else
	{
